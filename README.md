...
- **adminbot/** – Telegram-бот, который управляет профилями WattAttack, клиентами и инвентарём:
  ...
  - `/uploadclients [truncate|dry-run]` – импорт клиентов из CSV (ответом или пересылкой); режимы выбираются кнопками.
  ...
  - `/uploadschedule [keep] [dry-run]` – импорт расписания из XLSX.
  ...
  - Инлайн-меню и быстрый текстовый поиск (только для админов).
...
- **Управление клиентами**: CSV импорт нормализует имя, пол, вес, рост, FTP, педали, цели и т.д. `/client` и обычный текст ищут по клиентам. Карточка клиента показывает подобранные велосипеды и станки (учитываются рост, оси, кассеты). `/stands` ведёт учёт станков и даёт редактировать оси/кассеты, `/bikes` показывает парк велосипедов и позволяет менять диапазон роста, `/layout` отображает расстановку. `/account` получает актуальные данные профиля WattAttack.
