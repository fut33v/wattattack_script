...
- Уведомлять администраторов о новых бронированиях через клиентский бот.

## Основные модули / компоненты
- `adminbot/` – основной бот (команды, inline-навигация, поиск клиентов, управление администраторами).
- `krutilkavnbot/` – клиентский бот для бронирования слотов и управления профилем.
- `repositories/layout_repository.py` – привязка велосипедов к станкам.
- `scheduler/` – пакет со скедулером и нотификатором новых тренировок.
...
- `repositories/schedule_repository.py` – недели, временные слоты и бронирования расписания.
- `repositories/instructors_repository.py` – справочник инструкторов, привязка к слотам.
- `scripts/import_schedule_from_xlsx.py` – импорт расписания из Excel с обработкой алиасов клиентов.
- `webapp/main.py` (FastAPI) + `webapp/frontend/` (React/Vite SPA) – админка «Крутилка» с разделами для клиентов, инвентаря и расписания.
...
- Таблицы `schedule_weeks`, `schedule_slots`, `schedule_reservations`, `schedule_instructors` – недельные шаблоны, временные слоты, записи пользователей и инструкторы.
...
- Структуры фронтенда: `ScheduleWeekRow`, `ScheduleSlot`, `ScheduleReservation`, `InstructorRow` (см. `webapp/frontend/src/lib/types.ts`).
...
- Нотификатор: планировщик (`scheduler/scheduler.py`) циклически вызывает `scheduler/notifier.py`, который обращается к API WattAttack и БД, затем отправляет сообщения через Telegram API.
- Данные хранятся в PostgreSQL; импорт CSV обновляет таблицы; бот/нотификатор читают их через репозитории.
- Веб-приложение «Крутилка»: FastAPI (`webapp/main.py`) подаёт REST API (`/api/...`) и статический React-фронтенд. React SPA использует React Query, имеет две страницы расписания – overview (`/app/schedule`) и manage (`/app/schedule/manage`). На старте приложения выполняется `ensure_instructors_table()` для автосоздания таблицы инструкторов и дефолтного списка.
- Расписание: недели можно создавать, копировать, синхронизировать со станками, заполнять по шаблону, удалять; в слотах поддерживаются режимы «самокрутка»/«с инструктором», выбор инструктора, кол-во мест = числу станков. Импортер из Excel дополняет данные и обновляет имеющиеся недели (dry-run режим поддержан).
- Уведомления администраторов: при бронировании через `krutilkavnbot` автоматически отправляются уведомления всем администраторам с информацией о клиенте, дате/времени, типе занятия (самокрутка или с инструктором), станке и велосипеде.
...
- Веб-интерфейс оформлен в едином стиле (React + CSS), поля и списки стараемся делать компактными и читабельными (много контента сворачивается по клику).
...
- Развивать раздел расписания: компактный вывод слотов (время одной строкой, статистика занятости снизу), отображение имени инструктора/участника, управление неделями (создание, копирование, удаление, заполнение по шаблону).
- Поддерживать импорт Excel-расписаний: корректный маппинг имён клиентов (алиасы), создание прошлых недель, обновление существующих с учётом флага `replace`.
- Обеспечить надёжную систему уведомлений администраторов о новых бронированиях с указанием типа занятия.
